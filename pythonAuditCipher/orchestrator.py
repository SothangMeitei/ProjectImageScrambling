...
class CipherAuditSuite:
    ...
    def run_correlation_audit(self, plot_out: str = "outputs/correlation", silent: bool = False) -> dict:
        """Computes correlation coefficients, saves scatter plots, and returns values as a dict."""
        results = {}
        for d, short in [("horizontal", "h"), ("vertical", "v"), ("diagonal", "d")]:
            p_corr = CorrelationAnalyzer.calculate(self.plain,  d)
            c_corr = CorrelationAnalyzer.calculate(self.cipher, d)
            results[f"{short}_plain"]  = p_corr
            results[f"{short}_cipher"] = c_corr
            if not silent:
    ...
    def stage_attacks(self, crop_out: str, noise_out: str):
        from .robustness import CropConfiguration
        config = CropConfiguration(mode="random", num_boxes=3, size_range=(50, 200))
        RobustnessAnalyzer.stage_advanced_cropping(self.cipher, config, crop_out)
    ...
    def run_entropy_audit(self, plot_out: str = "outputs/histogram.png", silent: bool = False) -> float:
        """Computes entropy, saves histogram, stores local entropy for report access, and returns global entropy."""
        g_ent = EntropyAnalyzer.calculate_global(self.cipher)
        l_ent = EntropyAnalyzer.calculate_local(self.cipher)
        self._last_local_entropy = l_ent
        if not silent:
    ...
    @staticmethod
    def run_key_sensitivity_audit(cipher_path_key1: str, cipher_path_key2: str):
